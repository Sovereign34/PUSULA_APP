...
function validateExecutorOutput(output) {
  ...
  if (output.decision_type !== "ig_campaign_brief") {
    errors.push('decision_type "ig_campaign_brief" olmalı');
  }
  ...
  if (cb === null || typeof cb !== "object") {
    ...
    if (cb.placement !== "instagram_only") {
      errors.push('campaign_brief.placement "instagram_only" olmalı (MASTER_PLAN §3.2)');
    }
...
function runExecutorNode(request, llmClient) {
  ...
  const validationErrors = validateExecutorOutput(parseResult.output);
